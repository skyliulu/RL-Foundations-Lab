export const PPO_SAMPLES = [
  { id: 'A', advantage: 1.35, direction: 1, oldProbability: 0.18, token: 'because', reward: 1.8 },
  { id: 'B', advantage: 0.72, direction: -0.55, oldProbability: 0.31, token: 'therefore', reward: 1.2 },
  { id: 'C', advantage: 0.21, direction: 0.45, oldProbability: 0.24, token: 'verify', reward: 0.7 },
  { id: 'D', advantage: -0.28, direction: 0.7, oldProbability: 0.42, token: 'guess', reward: 0.1 },
  { id: 'E', advantage: -0.84, direction: -0.65, oldProbability: 0.36, token: 'ignore', reward: -0.8 },
  { id: 'F', advantage: -1.42, direction: -1, oldProbability: 0.12, token: 'fabricate', reward: -1.6 },
]

export function evaluatePpo({ clip = 0.2, updateStrength = 0.28, klBeta = 0.08 } = {}) {
  const samples = PPO_SAMPLES.map((sample) => {
    const ratio = Math.exp(updateStrength * sample.direction)
