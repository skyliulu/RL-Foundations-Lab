  ...
  return (
    <section className="token-mdp-lab" aria-label={content.figure}>
      ...
      <div className="token-mdp-controls">
        <fieldset><legend>{zh ? '选择一条 response' : 'Choose a response'}</legend>{options.map((item) => <button type="button" className={variant === item.id ? 'active' : ''} aria-pressed={variant === item.id} key={item.id} onClick={() => setVariant(item.id)}>{item.label}</button>)}</fieldset>
