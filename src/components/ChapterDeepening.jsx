    ...
    <section className="chapter-deepening" aria-label={zh ? '完整理论与算法脉络' : 'Complete theory and algorithm spine'}>
      {sections.map((section) => (
        <article className="deepening-section" key={section.id}>
          <header><span><MathText>{section.kicker}</MathText></span><h3><MathText>{section.title}</MathText></h3></header>
          {section.paragraphs.map((paragraph) => <p key={paragraph}><MathText>{paragraph}</MathText></p>)}
          {section.formulas?.length > 0 && (
            <div className="deepening-formulas">
              {section.formulas.map((formula, index) => (
                <div className="deepening-formula-step" key={formula}>
                  {section.formulaLabels?.[index] && <span><MathText>{section.formulaLabels[index]}</MathText></span>}
                  <MathFormula block latex={formula} />
                </div>
              ))}
            </div>
          )}
