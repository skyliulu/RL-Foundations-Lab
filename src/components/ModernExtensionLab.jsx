...
function DpoLab({ lang }) {
  ...
  return <div className="modern-lab-body">
    <div className="modern-controls"><label><span><MathFormula latex={String.raw`\beta`} /> <output>{beta.toFixed(2)}</output></span><input type="range" min="0.05" max="1.2" step="0.05" value={beta} onChange={(event) => setBeta(Number(event.target.value))} /></label><label><span>{lang === 'zh' ? '偏好对比强度' : 'Pair contrast'} <output>{contrast.toFixed(2)}</output></span><input type="range" min="-0.4" max="1.2" step="0.05" value={contrast} onChange={(event) => setContrast(Number(event.target.value))} /></label></div>
