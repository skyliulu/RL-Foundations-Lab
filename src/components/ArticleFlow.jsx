...
function FlowSection({ block }) {
  return <article className={`article-flow-block article-flow-section flow-${block.id}`}>
    ...
    {block.formulas?.length > 0 && <div className={`article-flow-equations${block.formulaAfter ? ' has-after' : ''}`}>{block.formulas.map((formula) => <ResponsiveMathFormula block className={`article-flow-equation is-${formula.role || 'support'}`} latex={formula.latex} narrowLatex={formula.narrowLatex} key={formula.latex} />)}</div>}
