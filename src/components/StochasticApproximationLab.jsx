...
export default function StochasticApproximationLab({ lang, content }) {
  ...
  return <section className="sa-lab" aria-label={content.figure}>
    ...
    <div className="sa-controls">
      <fieldset><legend>{zh ? '目标类型' : 'Target type'}</legend><div><button type="button" className={!drifting ? 'active' : ''} aria-pressed={!drifting} onClick={() => setDrifting(false)}>{zh ? '固定目标' : 'Stationary'}</button><button type="button" className={drifting ? 'active' : ''} aria-pressed={drifting} onClick={() => setDrifting(true)}>{zh ? '中途漂移' : 'Mid-run drift'}</button></div></fieldset>
      ...
      <fieldset><legend>{zh ? '每步 batch' : 'Batch per update'} <MathFormula latex={String.raw`m`} /></legend><div>{[1, 5, 20].map((size) => <button type="button" className={batchSize === size ? 'active' : ''} aria-pressed={batchSize === size} onClick={() => setBatchSize(size)} key={size}>{size}</button>)}</div></fieldset>
    </div>
