...
export const optimalityChapter = assertFoundationChapterDefinition({
  ...
  zh: {
    ...
    prelude: [
      { id: 'evaluation-control', kicker: '两个不同问题', title: '策略评估问“这套策略值多少”', paragraphs: ['当 π 固定时，状态 s 的价值由 π 在各动作上的概率和环境响应共同决定。', '确定性固定策略只选择一个动作，因此期望算子在该状态上读取这个动作的 qπ(s,a)。'], formulas: ['TπV(s) = Σₐ π(a|s) Σₛ′,ᵣ p(s′,r|s,a)[r + γV(s′)]'] },
      { id: 'max-reduction', kicker: '把策略也纳入优化', title: '最优性问“哪一个动作能给出最大 target”', paragraphs: ['任何随机策略产生的值都是各动作 q 的凸组合，不可能超过其中最大的一个。', '因此最大值可由确定性贪心策略达到：把概率 1 放在 arg max 动作上。'], formulas: ['T*V(s) = maxₐ Σₛ′,ᵣ p(s′,r|s,a)[r + γV(s′)]'] },
    ],
    sections: [
      { id: 'fixed-point-optimality', kicker: '存在、唯一与最优', title: '最优算子的不动点就是 V*', paragraphs: ['当 0 < γ < 1 时，Bellman 最优算子仍是压缩映射，因此存在唯一不动点。', '对这个 V* 逐状态取贪心动作，就得到至少一个确定性最优策略。'], formula: 'V* = T*V*,  π*(s) ∈ arg maxₐ q*(s,a)' },
      { id: 'reward-horizon', kicker: '策略为何改变', title: '环境、奖励与 γ 共同决定最优动作', paragraphs: ['课件基线下，远期反复进入目标的收益可能值得承受一次禁区 −1；这不是“最优策略喜欢风险”，而是累计回报的比较结果。', '缩短时间尺度或把禁区惩罚改为 −10，都可能让同一状态的最大动作改变。'], formula: 'q*(s,a) = E[Rₜ₊₁ + γV*(Sₜ₊₁) | s,a]' },
      { id: 'ties-transfer', kicker: '边界与迁移', title: '最优策略可以不唯一，最优价值仍唯一', paragraphs: ['若多个动作拥有相同最大 q 值，它们都可进入最优策略；确定性选择只是其中一种实现。', '下一章将把 T* 反复作用于整张价值表，并比较 Value Iteration 与 Policy Iteration 怎样到达同一不动点。'], formula: 'arg maxₐ q*(s,a) 可能包含多个动作' },
    ],
