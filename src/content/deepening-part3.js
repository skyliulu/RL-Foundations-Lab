...
export const vfaDeepeningZh = [
  ...
  {
    id: 'linear-sharing', kicker: '线性特征', title: '特征内积直接决定一次更新会传播到哪些状态',
    paragraphs: ['在线性近似中，预测是特征向量与参数的内积，因此当前状态的特征就是参数更新方向。更新以后，另一个状态的预测变化等于两个状态特征的内积乘本次误差：特征正交就互不影响，特征相似就一起变化。', '独热特征让不同状态彼此正交，恰好退化为表格方法。分块编码、傅里叶基与神经网络虽然形式不同，本质上都在规定哪些状态共享参数。'],
  ...
  {
    id: 'approximate-control', kicker: '从预测进入控制', title: '策略随价值估计一起变化时，目标与数据分布都会移动',
    paragraphs: ['预测问题在固定策略下逼近一个固定的价值函数；控制问题却一边用动作价值选择行为，一边更新动作价值。参数变化会改变行为策略，行为策略又会改变训练数据与自举目标。', '函数近似、自举和策略外学习同时出现时，常被称为“致命三元组”。它并不意味着训练必然发散，而是说明表格方法的收敛直觉不再自动成立。下一章将用目标网络和经验回放分别减慢目标变化与重排数据。'],
...
export const dqnDeepeningZh = [
  {
    id: 'moving-target', kicker: '具体不稳定来源', title: '同一网络既预测又出题，连续轨迹还会反复强化局部误差',
    paragraphs: ['若用同一个在线网络预测当前动作价值并构造下一状态的最大动作价值，一次参数更新会同时改变被拟合项与监督标签。优化器刚靠近旧目标，目标本身就跟着移动。', '与此同时，按轨迹顺序到来的相邻样本高度相关，连续几步梯度可能都偏向同一小片状态。DQN 因而面对两个独立问题：目标变化太快，以及训练证据缺少重排。下面的推导分别引入滞后的目标网络与经验回放。'],
  ...
  {
    id: 'replay-why', kicker: '机制带来的新权衡', title: '经验回放降低时间相关，也会改变样本的新鲜度与复用次数',
    paragraphs: ['环境每一步产生一条转移并写入缓冲区，训练批次再从整个窗口随机抽取。这样，相邻的梯度不必来自相邻时刻，旧经验也能被多次利用。', '代价是缓冲区混合了不同时期的行为策略，并不等于当前策略的数据分布。容量太小会退回高度相关的在线训练；容量太大又可能保留大量过时经验。优先回放还会改变采样概率，需要额外权重控制偏差。'],
  ...
  {
    id: 'dqn-complete', kicker: '完整算法', title: '行动、入库、梯度更新与目标同步按四个节奏运行',
    paragraphs: ['行动策略根据在线动作价值作 ε-greedy 选择；环境转移立即写入缓冲区；梯度更新从缓冲区抽取小批量；目标网络只按固定周期或缓慢平均进行同步。', '评估时通常关闭探索并使用在线网络。训练回报、时序差分误差、动作价值尺度和同步时的跳变必须一起观察，因为较低的损失也可能对应错误但自洽的高估。'],
