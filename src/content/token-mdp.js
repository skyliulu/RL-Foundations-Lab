...
const zhDerivation = [
  ...
  { id: 'termination', rule: '分别记录真正终止与长度截断', latex: String.raw`\begin{aligned}D_{t+1}^{\mathrm{term}}&=\mathbf 1[A_t=\mathrm{EOS}],\\D_{t+1}^{\mathrm{trunc}}&=\mathbf 1[t+1=T_{\max}\land A_t\ne\mathrm{EOS}].\end{aligned}`, short: '一条回答会停止，但停止原因决定是否保留后续价值。', detail: 'EOS 是策略选择的终止动作，此后同一回答没有后续状态价值；长度截断只是环境预算边界，边界外仍可能存在可自举的延续价值。训练数据必须分开保存两个标记。', assumptions: [String.raw`T_{\max}<\infty`], symbols: [[String.raw`D_{t+1}^{\mathrm{term}}`, '真正终止标记'], [String.raw`D_{t+1}^{\mathrm{trunc}}`, '长度截断标记']] },
...
const enDerivation = [
  ...
  { id: 'termination', rule: 'Record true termination and length truncation separately', latex: String.raw`\begin{aligned}D_{t+1}^{\mathrm{term}}&=\mathbf 1[A_t=\mathrm{EOS}],\\D_{t+1}^{\mathrm{trunc}}&=\mathbf 1[t+1=T_{\max}\land A_t\ne\mathrm{EOS}].\end{aligned}`, short: 'A response stops, but the reason determines whether continuation value remains.', detail: 'EOS is a policy-chosen terminal action with no later state in the same response. A length boundary is an external budget cutoff and can retain bootstrap value beyond the boundary. Training data must preserve both flags.', assumptions: [String.raw`T_{\max}<\infty`], symbols: [[String.raw`D_{t+1}^{\mathrm{term}}`, 'true-terminal flag'], [String.raw`D_{t+1}^{\mathrm{trunc}}`, 'length-truncation flag']] },
...
export const tokenMdpChapter = assertFoundationChapterDefinition({
  id: 'tokenmdp', sources,
  zh: {
    prerequisite: '前置：MDP、return、策略梯度与 PPO', summaryTitle: '语言模型不是类比成 MDP，而是逐项定义成 token 级决策过程', eyebrow: '语言模型强化学习 · Token MDP', title: '将 response 定义为 token 级强化学习轨迹', intro: '从 prompt 开始，语言模型反复观察当前前缀、选择下一个 token，并把 token 追加到上下文。只有把状态、动作、转移、终止与奖励逐一说明，PPO 中的 old probability、value 和 advantage 才有明确对应对象。', derivationIntro: '先完成从生成过程到 MDP 五要素的严格映射，再讨论偏好奖励与系统实现。', bridge: '本章界定语言生成中的强化学习对象；下一章再加入 reward model、reference model 与分布式 rollout。', experimentIntro: '在轨迹实验中选择一条 response，逐位置检查 shaped reward、return、loss mask、bootstrap mask 与终止原因。长度截断时还可改变边界之外的 continuation value，观察它怎样进入最后一个 value target。', interpretation: '先检查 token 是由哪一个 behavior policy 采样，再检查 reward 与两个 mask 放在哪个位置。EOS 表示同一 response 已真正结束，bootstrap 必须归零；长度截断只是外部预算边界，仍要保留可能的后续价值。', figure: '交互图 15.1 · Token Trajectory Lab', instruction: '选择一条 response，逐 token 检查 rollout 证据、两个 mask 与截断后的 continuation value', question: '什么信息必须进入状态和 rollout 记录，才能让 token 级 PPO 数学成立？', derivation: zhDerivation, deepening: zhDeepening,
