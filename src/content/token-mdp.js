...
export const tokenMdpChapter = assertFoundationChapterDefinition({
  id: 'tokenmdp', sources,
  zh: {
    prerequisite: '前置：MDP、return、策略梯度与 PPO', summaryTitle: '语言模型不是类比成 MDP，而是逐项定义成 token 级决策过程', eyebrow: '语言模型强化学习 · Token MDP', title: '一个 response 为什么可以被看成一条强化学习轨迹？', intro: '从 prompt 开始，语言模型反复观察当前前缀、选择下一个 token，并把 token 追加到上下文。只有把状态、动作、转移、终止与奖励逐一说明，PPO 中的 old probability、value 和 advantage 才有明确对应对象。', derivationIntro: '先完成从生成过程到 MDP 五要素的严格映射，再讨论偏好奖励与系统实现。', bridge: '这一章只回答“语言生成里的强化学习对象是什么”；下一章才把 reward model、reference model 与分布式 rollout 加进来。', experimentIntro: '在下方轨迹实验中逐 token 前进，切换终局奖励、过程奖励与 KL 强度，观察每个位置的 shaped reward、return 与终止原因。', interpretation: '先检查 token 是由哪一个 behavior policy 采样，再检查 reward 放在哪个位置。总分相同并不意味着相同的信用分配，EOS 与长度截断也不能使用同一终止解释。', figure: '交互图 15.1 · Token Trajectory Lab', instruction: '逐 token 选择 response，并调整奖励组成', question: '什么信息必须进入状态和 rollout 记录，才能让 token 级 PPO 数学成立？', derivation: zhDerivation, deepening: zhDeepening,
    prelude: [{ id: 'bridge', kicker: '从经典环境切换对象', title: '网格状态换成前缀，方向动作换成 token', paragraphs: ['MDP 的数学对象没有改变，改变的是每个对象在语言生成中的含义。先做严格映射，可以避免把 reward model、reference model 或 tokenizer 混成环境的一部分。'], formulas: [String.raw`\text{grid cell}\mapsto\text{prefix},\qquad\text{move}\mapsto\text{next token}`] }],
    sections: [{ id: 'sampling-policy', kicker: '行为策略', title: '解码配置属于采样策略', paragraphs: ['temperature、top-k 与 top-p 改变 logits 到实际采样概率的映射。PPO old log-probability 必须对应真实采样分布，而不是另一个未经截断的理论分布。'], formula: String.raw`b_t(a\mid s)\propto\exp(z_a/\tau)\mathbf 1[a\in\mathcal V_t]` }, { id: 'terminal-vs-truncated', kicker: '终止语义', title: 'EOS 与长度截断对 value bootstrap 的含义不同', paragraphs: ['EOS 表示策略选择结束回答；长度边界表示外部预算强制停止。是否把截断位置视为真正 terminal，会改变 return 与 value target。'], formula: String.raw`D_t^{\mathrm{EOS}}\neq D_t^{\mathrm{truncation}}` }, { id: 'next', kicker: '下一步', title: '从 token MDP 进入 RLHF 系统', paragraphs: ['有了稳定的 token trajectory contract，reward、reference、value 与 PPO 才能在同一批位置上组合。'] }],
  ...
  en: {
    prerequisite: 'Prerequisites: MDPs, returns, policy gradient, and PPO', summaryTitle: 'A language model is defined token by token as an MDP, not merely compared to one', eyebrow: 'Language-model reinforcement learning · Token MDP', title: 'Why can one response be treated as a reinforcement-learning trajectory?', intro: 'Starting from a prompt, the model observes its prefix, selects a token, and appends it to context. PPO old probability, value, and advantage become meaningful only after state, action, transition, termination, and reward are mapped explicitly.', derivationIntro: 'Complete the five-part MDP mapping before adding preference reward or systems concerns.', bridge: 'This chapter asks what the RL objects are in generation. The next adds reward, reference, and distributed rollout.', experimentIntro: 'Advance token by token, vary terminal reward, process reward, and KL strength, and inspect shaped reward, return, and termination reason.', interpretation: 'Identify the behavior policy that sampled each token before reading reward placement. Equal total score does not imply equal credit assignment, and EOS differs from length truncation.', figure: 'Interactive Figure 15.1 · Token Trajectory Lab', instruction: 'Build a response token by token and vary reward composition', question: 'What must enter state and rollout records for token-level PPO to be valid?', derivation: enDerivation, deepening: enDeepening,
    prelude: [{ id: 'bridge', kicker: 'Change the objects', title: 'Grid state becomes prefix; move action becomes token', paragraphs: ['The MDP mathematics stays fixed while each object receives a language-generation meaning. Strict mapping prevents reward, reference, or tokenizer roles from being conflated.'], formulas: [String.raw`\text{grid cell}\mapsto\text{prefix},\qquad\text{move}\mapsto\text{next token}`] }],
    sections: [{ id: 'sampling-policy', kicker: 'Behavior policy', title: 'Decoding configuration is part of the sampling policy', paragraphs: ['Temperature, top-k, and top-p change the mapping from logits to actual probabilities. PPO old probability must match the distribution that truly sampled the token.'], formula: String.raw`b_t(a\mid s)\propto\exp(z_a/\tau)\mathbf 1[a\in\mathcal V_t]` }, { id: 'terminal-vs-truncated', kicker: 'Termination semantics', title: 'EOS and length truncation differ for value bootstrap', paragraphs: ['EOS is a policy choice to end; a length boundary is an external budget. Treating both as true terminal changes returns and value targets.'], formula: String.raw`D_t^{\mathrm{EOS}}\neq D_t^{\mathrm{truncation}}` }, { id: 'next', kicker: 'Next', title: 'From token MDP to the RLHF system', paragraphs: ['A stable token trajectory contract lets reward, reference, value, and PPO align on one batch.'] }],
