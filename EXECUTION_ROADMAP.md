...
- 文档版本：`v0.2`
- 日期：`2026-07-19`
- 状态：M3 经典已知模型主线完成，进入 M4 Model-free Learning 主线
...
Part I 的 01–05 已形成从 MDP、Return/Value、Bellman 期望方程到最优性与规划的连续路径；阅读壳、实验协议、共享 Grid World、seeded sampling 与 Step Microscope 状态契约现在作为后续章节的实施基线。
...
- 06–12 的 model-free、function approximation 与 policy-gradient 主线尚未实现。
- 跨章节通用的 episode/trajectory 数据模型、多 seed 汇总和统计区间尚未实现。
- PPO 样本尚未来自可推进的 rollout/update 状态机。
...
M3 已完成并通过 `PART_I_ACCEPTANCE.md` 审计。下一步进入 **M4：Model-free Learning 主线**：先冻结章节 06 的 episode/trajectory 与 seed protocol，再实现 Monte Carlo Episode Tape，使“已知模型的精确期望”自然过渡到“只从经验样本估计”。
