...
- **16 PPO-based RLHF 系统**：追踪同一批 token 如何穿过 policy、old policy、reference、reward、value、GAE 与 PPO 更新。
- **17 直接偏好优化（DPO）**：从 KL 正则化最优策略推导成对偏好目标，并定位隐式奖励。
- **18 组相对策略优化（GRPO）**：结合在线可验证奖励、组相对优势与 DAPO 类稳定化机制。

### 第五部分 · Coding 与长程 Agent

- **19 Coding RL**：把编译、测试、隐藏用例与迭代修复组织成可执行反馈。
- **20 Agent MDP**：把多轮工具、观察、记忆、预算与终止条件建模为一条轨迹。
- **21 长程信用分配**：比较终局、折扣、过程、分段与事后归因。
...
- Token Trajectory Lab 与 PPO-based RLHF 共享 batch 系统图；
- DPO 偏好对、GRPO 成组 rollout、代码执行、多轮 Agent 与长程信用的专属实验。
...
01–21 章均已形成可运行的双语学习路径，经典基础、无模型方法、函数近似、语言模型后训练、Coding Agent 与长程信用分配已经连成一条连续主线。部分中段章节的交互仍在从参数实验继续加强为更细粒度的算法内部状态可视化。
