...
- When an interaction represents a policy or environment rollout, draw the traversed state path directly on the environment map and synchronize it with the trajectory tape, selected run, current state, and reset behavior. If an expectation-based interaction shows only a selected, sampled, or most-likely branch, label that distinction explicitly; never present backup order or an enumerated branch set as an environment trajectory.
...
- For every rollout-style grid interaction, verify that the map path, trajectory list or tape, selected run, current-state marker, and reset action remain synchronized in both languages. For expectation diagrams, verify that any displayed path is identified as a sample, selection, or most-likely branch and that alternative probability mass remains visible elsewhere.
